...
struct IndexerMetadataKernel {
  // NOTE: the reserved dynamic shared memory is (kMaxBatchSizeInSmem+1)*4 bytes.
  // Datacenter GPUs (Hopper/Blackwell B-series) allow >200KB opt-in smem, but
  // workstation Blackwell (sm_120, e.g. RTX PRO 5000) caps opt-in smem at ~99KB.
  // 16384*2 -> 128KB exceeds that cap and makes cudaFuncSetAttribute fail with
  // "invalid argument". Use 16384 (=64KB reserve) which fits sm_120 and still
  // covers realistic batch sizes; larger batches fall back to global memory via
  // the use_smem check below.
  static constexpr auto kMaxBatchSizeInSmem = 16384;  // 64 KB smem reserve

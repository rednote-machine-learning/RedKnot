...
def apply_deepseek_v4_defaults(server_args: "ServerArgs", model_arch: str) -> None:
    ...
    if server_args.attention_backend not in {"redknot_mla"}:
        server_args.attention_backend = "dsv4"
    server_args.page_size = 256
    logger.info(
        f"Use {server_args.attention_backend} attention backend for {model_arch}, "
        "setting page_size to 256."
    )

    if getattr(server_args, "redknot_mla_profile_enable", False):
        # The MLA head profiler hooks the non-fused ``_forward_prepare`` path,
        # which only runs when the multi-stream overlap fast path is disabled.
        if envs.SGLANG_OPT_USE_MULTI_STREAM_OVERLAP.get():
            envs.SGLANG_OPT_USE_MULTI_STREAM_OVERLAP.set(False)
            logger.warning(
                "RedKnot MLA profiler: disabling multi-stream overlap so the "
                "non-fused attention path (with latent KV) is exercised."
            )
        if server_args.cuda_graph_max_bs is None or server_args.cuda_graph_max_bs > 0:
            server_args.disable_cuda_graph = True
            logger.warning(
                "RedKnot MLA profiler: disabling CUDA graph for the analysis run."
            )

...
class DeepseekV4AttnBackend(
    ...
    # ──────────────────────────────────────────────────────────────────
    # RedKnot offline MLA reuse (demo)
    # ──────────────────────────────────────────────────────────────────
    def _maybe_redknot_reuse_hook(self, layer_id, forward_batch, token_to_kv_pool):
        """Snapshot / relocate per-segment SWA rope KV for offline reuse.

        Reads ``forward_batch.redknot_reuse_plan`` (a dict installed by the
        scheduler from the request). The plan has one of:
          {"mode": "snapshot", "seg_hash": str, "length": int}
              -> store this single-segment prefill's rope KV.
          {"mode": "relocate", "segments": [
                 {"seg_hash": str, "global_offset": int, "length": int,
                  "skip_first": int}], ...}
              -> for each reused segment, re-rotate its cached rope from local to
                 global positions and write into the segment's token slots.

        Token slots for the (single) request in the batch are taken from
        ``req_to_token[req_pool_index, :seq_len]``.
        """
        import os
        ...
        # Only act on prefill of a single-request batch (demo scope).

...
@register_attention_backend("redknot")
def create_redknot_backend(runner):
    """RedKnot: head-classified KV compression backend.

    Loads optional head configuration from
    ``runner.server_args.redknot_head_config_path``. If absent, the backend
    runs as a plain SDPA fallback (i.e. behaves like TorchNativeAttnBackend)
    until a config is attached.
    """
    from sglang.srt.layers.attention.redknot_backend import RedKnotAttnBackend

    q_chunk = getattr(runner.server_args, "redknot_q_chunk_size", 2048)
    kernel = getattr(runner.server_args, "redknot_kernel", "fa2")
    use_segpaged = getattr(runner.server_args, "redknot_segpaged_decode", False)
    page_size = getattr(runner.server_args, "redknot_page_size", 64)
    return RedKnotAttnBackend(
        runner,
...
@register_attention_backend("segpaged")
def create_segpaged_backend(runner):
    """SegPagedAttention backend: per-head paged KV + varlen attention."""
    from sglang.srt.layers.attention.segpaged_backend import SegPagedAttnBackend

    q_chunk = getattr(runner.server_args, "segpaged_q_chunk_size", 2048)
    kernel = getattr(runner.server_args, "segpaged_kernel", "fa2")
    page_size = getattr(runner.server_args, "segpaged_page_size", 64)
    return SegPagedAttnBackend(
        runner,
        q_chunk_size=q_chunk,
        kernel=kernel,
        page_size=page_size,
...
@register_attention_backend("redknot_mla")
def create_redknot_mla_backend(runner):
    from sglang.srt.layers.attention.redknot_mla_backend import RedKnotMLAAttnBackend

    logger.info("Using RedKnotMLAAttnBackend for DeepSeek V4 MLA attention.")
    return RedKnotMLAAttnBackend(runner)

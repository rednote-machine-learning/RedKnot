...
if is_blackwell_supported() and is_flashinfer_available():
    ...
    # mm_mxfp8 / mxfp8_quantize are only present in newer flashinfer builds and
    # are only needed for the mxfp8 GEMM path. Degrade gracefully when missing
    # so DeepSeek-V4 (fp8 e4m3) can still start on Blackwell with older flashinfer.
    try:
        ...
        _HAS_FLASHINFER_MXFP8 = True
    except ImportError:
        SfLayout = None
        _raw_flashinfer_mm_mxfp8 = None
        _raw_flashinfer_mxfp8_quantize = None
        _HAS_FLASHINFER_MXFP8 = False
...
def flashinfer_deepgemm_w8a8_block_fp8_linear_with_fallback(
    ...
    if not (
        shape_supported and dtype_supported and fp8_blockscale_gemm_sm90 is not None
    ):

...
if is_flashinfer_available():
    # NOTE: ``trtllm_mxint4_block_scale_moe`` and friends only exist in newer
    # flashinfer builds. Older installs (e.g. 0.5.3) lack them. This MoE scheme
    # is only used for mxint4-quantized MoE models, so for everything else
    # (e.g. dense Llama-70B) we tolerate the missing symbols and leave them as
    # ``None`` instead of crashing the whole quantization import chain.
    try:

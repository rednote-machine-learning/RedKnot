...
class Envs:
    ...
    # RedKnot offline MLA KV reuse (demo): when enabled, the dsv4 backend
    # snapshots per-segment SWA rope KV and supports RoPE-relocated reuse so a
    # document segment prefilled once (at local positions) can be spliced into
    # later concatenated requests at arbitrary global offsets.
    SGLANG_REDKNOT_OFFLINE_REUSE = EnvBool(False)

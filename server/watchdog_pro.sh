#!/bin/bash
# watchdog_pro.sh —— DeepSeek-V4-Pro server 看门狗
#
# 用途: 防止推理任务挂掉后 GPU 闲置。每隔 INTERVAL 秒检查一次 server 进程:
#   - 进程在跑           -> 跳过
#   - 进程不在(挂了/退出) -> 自动重启 DeepSeek-V4-Pro 推理 server
#
# 说明: 本脚本只做"重启推理服务", 不做 GPU 空转占卡 (gpu_burn 之类滥用资源的行为)。
#
# 用法:
#   nohup setsid bash server/watchdog_pro.sh > /tmp/watchdog_pro.log 2>&1 &
# 停止:
#   touch /tmp/watchdog_pro.stop   # 看门狗下个周期会优雅退出

set -uo pipefail

INTERVAL="${WATCHDOG_INTERVAL:-3600}"          # 检查间隔, 默认 1 小时
SERVER_DIR="$(cd "$(dirname "$0")" && pwd)"
START_SCRIPT="$SERVER_DIR/start_server_pro.sh"
SERVER_LOG=/tmp/pro_server.log
PROC_PATTERN="sglang.launch_server.*DeepSeek-V4-Pro"
STOP_FLAG=/tmp/watchdog_pro.stop
...
restart_server() {
    log "server 进程不存在, 重启 DeepSeek-V4-Pro 推理服务..."
    # 清理可能残留的半死进程

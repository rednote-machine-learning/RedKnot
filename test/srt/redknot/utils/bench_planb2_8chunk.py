#!/usr/bin/env python3
"""8-chunk x 2K RAG benchmark: Plan-2 RedKnot vs standard inference.

Qwen3.5-35B-A3B. Context = 8 ordered chunks of 2K tokens (~16K). Compares the
Plan-2 chunked driver (chunk1 exact prefix; chunk>=2 linear reuses offline state;
full layers head-class sparse at the Qwen3.5 sweet spot frac_global=0.40 /
window=4096) against standard full-recompute inference.

Metrics: F1/EM (quality), online TTFT + speedup, analytic full-attn prefill FLOPs
saving. The Plan-2 online TTFT EXCLUDES the offline capture pass (that is the
amortized cross-request RAG cost, analogous to RedKnot offline prefill).

Run:
  HF_ENDPOINT=https://huggingface.co PYTHONPATH=python:<venv-sp>:<sys-sp> \
    CUDA_VISIBLE_DEVICES=0,1,2,3 .venv_tf5/bin/python \
    test/srt/redknot/bench_planb2_8chunk.py
"""
...
def full_attn_saving(frac, window, T):
    dense = T * (T + 1) / 2.0
    hc = frac * dense + (1 - frac) * T * min(window, T)
    return 1.0 - hc / dense
...
def main():
    ...
    W = 96
    print("=" * W)
    print(f" 8-CHUNK x 2K RAG: Plan-2 RedKnot vs standard — {Path(MODEL).name}")
    print(
        f" {N_CHUNK} chunks x {CHUNK} = {T} tok | frac_global={FRAC} window={WINDOW} N={N}"
    )
    print(" (RedKnot online TTFT excludes the amortized offline capture pass)")
    print("=" * W)
    rows = []
    for ds in DATASETS:
        ...
        for s in samples:
            ...
            rt += rttft
        k = len(samples)
        save = full_attn_saving(FRAC, WINDOW, T)
        rows.append((ds, k, sf / k, se / k, rf / k, re_ / k, st / k, rt / k, save))
        print(
            f" {ds:16} N={k} | std F1={sf / k:.3f} EM={se / k:.3f} TTFT={st / k:.2f}s "
            f"| RK F1={rf / k:.3f} EM={re_ / k:.3f} TTFT={rt / k:.2f}s "
            f"speedup={st / max(rt, 1e-3):.2f}x save={save * 100:.0f}%"
        )

    print("\n" + "=" * W)
    print(" SUMMARY (8-chunk x 2K RAG, Plan-2 vs standard)")
    print("=" * W)
    print(
        f" {'dataset':16} {'stdF1':>6} {'rkF1':>6} {'dF1':>6} {'stdTTFT':>8} {'rkTTFT':>8} {'speedup':>8} {'attnSave':>9}"
    )
    for ds, k, sf, se, rf, re_, st, rt, save in rows:
        print(
            f" {ds:16} {sf:6.3f} {rf:6.3f} {rf - sf:+6.3f} {st:7.2f}s {rt:7.2f}s {st / max(rt, 1e-3):7.2f}x {save * 100:8.1f}%"
        )
    if rows:
        asf = sum(r[2] for r in rows) / len(rows)
        arf = sum(r[4] for r in rows) / len(rows)
        ast = sum(r[6] for r in rows) / len(rows)
        art = sum(r[7] for r in rows) / len(rows)
        print("-" * W)
        print(
            f" AVG std F1={asf:.3f} RK F1={arf:.3f} (d={arf - asf:+.3f}) | "
            f"TTFT {ast:.2f}->{art:.2f}s ({ast / max(art, 1e-3):.2f}x) | "
            f"full-attn save~{full_attn_saving(FRAC, WINDOW, T) * 100:.0f}%"
        )
    print("=" * W)
